...
(
  ICAPACITY IN NUMBER , PATRONID IN VARCHAR2 , INPUTDT IN DATE, rmview OUT SYS_REFCURSOR)
IS
  patronType CHAR;
	userinputdt DATE;
	tabledatadt DATE;
...
BEGIN
  ...
  IF (patronType='S') THEN
    OPEN rmview FOR SELECT R.room_no, R.capacity, L.l_name, RC.checkouttime, RC.returntime
