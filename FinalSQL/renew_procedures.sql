...
-- Check whether in hold or not
 if(checkHold(patronId,varType)) then
      if(isQueueBook(bISBN)=false) then
        ...
        update books_checkout set returndate=localtimestamp where resource_id=bISBN and p_id=patronId and returndate is null;
        insert into books_checkout values(bISBN,LOCALTIMESTAMP,LOCALTIMESTAMP + INTERVAL '14' DAY,patronId,bLid,null,0,null,null);
      end if;  
 end if;
 commit;
end; 
...
-- Check whether in hold or not
 if(checkHold(patronId,varType)) then
      if(isQueueJournal(jISSN)=false) then
        ...
        insert into journals_checkout values(jISSN,LOCALTIMESTAMP,LOCALTIMESTAMP + INTERVAL '12' HOUR,patronId,jLid,null,0,null,null);
      end if;  
 end if;
 commit;
end; 
...
-- Check whether in hold or not
 if(checkHold(patronId,varType)) then
      if(isQueueConfp(cCONFNUM)=false) then
        ...
        insert into confp_checkout values(cCONFNUM,LOCALTIMESTAMP,LOCALTIMESTAMP + INTERVAL '12' HOUR,patronId,cLid,null,0,null,null);
      end if;  
 end if;
 commit;
end;
